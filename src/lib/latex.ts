...
const pdftex: PdfTeXEngine = new PdfTeXEngine()
const xetex: XeTeXEngine = new XeTeXEngine()
...
export default async function latex(texDoc: string, opts: LaTeXOpts) {
  ...
  switch (opts.cmd) {
    case 'pdflatex': {
      ...
      await pdftex.writeMemFSFile('main.tex', new Uint8Array(Buffer.from(texDoc)))
    ...
    case 'xelatex': {
      ...
      await xetex.writeMemFSFile('main.tex', new Uint8Array(Buffer.from(texDoc)))
